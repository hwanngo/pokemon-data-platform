...
class PokemonType(Base):
    ...
    def __repr__(self):
        return (
            f"<PokemonType(pokemon_id={self.pokemon_id}, type_id={self.type_id}, slot={self.slot})>"
        )
...
class TypeEffectiveness(Base):
    ...
    attack_type = relationship(
        "Type", foreign_keys=[attack_type_id], back_populates="attack_effectiveness"
    )
    defense_type = relationship(
        "Type", foreign_keys=[defense_type_id], back_populates="defense_effectiveness"
    )
